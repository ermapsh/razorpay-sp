...
public class SlidingWindowLuaLimiter implements RateLimiter {
    ...
    //Java version: ZCARD → check → ZADD are separate Redis operations,
    // so under concurrent requests, two requests can both see the same count and exceed the limit.
    //Lua version: remove → count → check → add → expire happens atomically inside Redis, preventing race conditions.
    //Lua executes the entire rate-limit operation atomically inside Redis,
    // while the Java version uses multiple Redis commands that are not atomic as a group, so concurrent requests can cause race conditions.
